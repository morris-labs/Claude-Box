...
// Runs one background `ssh -N` process implementing every -L/-R forward a
// box's record carries (BoxRecord::sshHost/sshIdentity/sshForwards).
//
...
// ever needs an ssh client or credentials of its own. One process per box,
// covering every forward that box has configured, since ssh happily
// multiplexes any number of -L/-R flags over a single connection.
//
...
public:
    ...
    bool start(const BoxRecord &rec);
