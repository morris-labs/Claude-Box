...
/* --- claude-box local patch (not upstream) ------------------------------
 * The Windows SDK's <rpcndr.h>, pulled in transitively by <windows.h> and
 * therefore by Qt's Windows headers, does `#define small char`. That turns
 * the `unsigned int small : 1;` bitfield in VTermScreenCellAttrs below into
 * `unsigned int char : 1;` and fails the build in any TU that includes both.
 * libvterm never wants that macro; drop it. Harmless elsewhere (it only ever
 * expands to `char`, and MIDL-generated code does not share a TU with this).
 */
#ifdef small
# undef small
#endif
